...
const DiaryPage = () => {
  ...
  // --- YENİ: Silme Onay Modalı Stateleri ---
  ...
  // --- YENİ: Silme Onay Modalı Fonksiyonları ---
  ...
  const confirmDelete = async () => {
    if (!productToDelete || !diaryId || !token) return;
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await instance.delete(`/diary/${diaryId}/${productToDelete}`, config);
  ...
  const handleSaveProduct = async (productData) => {
    ...
    try {
      ...
      if (editingItem) {
        await instance.patch(`/diary/update/${editingItem._id}`, { weight: productData.weight, date: formattedDate }, config);
      } else {
        const body = { date: formattedDate, productId: productData.productId, weight: productData.weight };
  ...
  return (
    <section className={styles.diarySection}>
      <div className={`${styles.diaryContent} ${!isLoggedIn ? styles.withBackground : ""}`}>
        <div className={styles.leftSide}>
          <DiaryDateCalendar selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
          <div className={styles.desktopFormWrapper}>
            <DiaryAddProductForm onSave={handleSaveProduct} isMobile={false} isModal={false} />
          </div>
          ...
          <button className={styles.mobileAddBtn} onClick={openAddModal} title="Add food">
            <PlusIcon />
          </button>
        </div>
      ...
      {isDeleteModalOpen && (
        <Modal onClose={() => setIsDeleteModalOpen(false)}>
          <div className={styles.confirmModal}>
            <h2 className={styles.confirmTitle}>Are you sure?</h2>
            <p className={styles.confirmText}>Do you really want to delete this product from your diary?</p>
